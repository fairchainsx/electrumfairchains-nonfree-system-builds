...
if [ -z "$1" ]; then
    echo "Usage: $0 electrumfairchains.app"
    exit -127
fi
...
if ! which ${genisoimage} > /dev/null 2>&1; then
	mkdir -p /tmp/electrumfairchains-macos
	cd /tmp/electrumfairchains-macos
...
rm -rf /tmp/electrumfairchains-macos/image > /dev/null 2>&1
mkdir /tmp/electrumfairchains-macos/image/
cp -r $1 /tmp/electrumfairchains-macos/image/
...
${genisoimage} \
    ...
    -l \
    -probe \
    -V "ElectrumFairChains" \
    -no-pad \
    -r \
    -dir-mode 0755 \
    -apple \
    -o ElectrumFairChains_uncompressed.dmg \
    /tmp/electrumfairchains-macos/image || fail "Unable to create uncompressed dmg"

dmg dmg ElectrumFairChains_uncompressed.dmg electrumfairchains-$VERSION.dmg || fail "Unable to create compressed dmg"
rm ElectrumFairChains_uncompressed.dmg

echo "Done."
sha256sum electrumfairchains-$VERSION.dmg
