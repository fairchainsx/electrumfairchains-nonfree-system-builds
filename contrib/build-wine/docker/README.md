...
2. Build image
    ...
    ```
    $ sudo docker build -t electrumfairchains-wine-builder-img contrib/build-wine/docker
    ```
...
3. Build Windows binaries
    ...
    $ FRESH_CLONE=contrib/build-wine/fresh_clone && \
        ...
        mkdir -p $FRESH_CLONE && \
        cd $FRESH_CLONE  && \
        git clone https://github.com/fairchainsx/electrumfairchains.git -b "3.3.4-fcx" && \
    ...
    $ git checkout $REV
    $ sudo docker run -it \
        --name electrumfairchains-wine-builder-cont \
        -v $PWD:/opt/wine64/drive_c/electrumfairchains \
        --rm \
        --workdir /opt/wine64/drive_c/electrumfairchains/contrib/build-wine \
        electrumfairchains-wine-builder-img \
        ./build.sh $(python3 -c "import electrumfairchains.version; print(electrumfairchains.version.ELECTRUMFAIRCHAINS_VERSION)")
    ```
...
|-----------------------------------------------------------------------------------------------|
| Upload signatures to 'electrumfairchains-signatures' repo, as `$version/$filename.$builder.asc`         |
