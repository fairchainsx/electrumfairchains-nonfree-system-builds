...
;Variables
  ...
  !define PRODUCT_NAME "ElectrumFairChains"
  !define PRODUCT_WEB_SITE "https://github.com/fairchainsx/electrumfairchains"
  !define PRODUCT_PUBLISHER "FairChains developers"
...
;General
  ...
  ;Name and file
  Name "${PRODUCT_NAME}"
  OutFile "dist/electrumfairchains-setup.exe"
...
;Interface Settings
  ...
  !define MUI_ICON "c:\electrumfairchains\electrumfairchains\gui\icons\efc.ico"
...
Section
  ...
  File /r "dist\electrumfairchains\*.*"
  File "c:\electrumfairchains\electrumfairchains\gui\icons\efc.ico"
  ...
  CreateShortCut "$DESKTOP\${PRODUCT_NAME}.lnk" "$INSTDIR\electrumfairchains-${PRODUCT_VERSION}.exe" ""

  ;Create start-menu items
  DetailPrint "Creating start-menu items..."
  CreateDirectory "$SMPROGRAMS\${PRODUCT_NAME}"
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\Uninstall.lnk" "$INSTDIR\Uninstall.exe" "" "$INSTDIR\Uninstall.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME}.lnk" "$INSTDIR\electrumfairchains-${PRODUCT_VERSION}.exe" "" "$INSTDIR\electrumfair-${PRODUCT_VERSION}.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME} Testnet.lnk" "$INSTDIR\electrumfairchains-${PRODUCT_VERSION}.exe" "--testnet" "$INSTDIR\electrumfairchains-${PRODUCT_VERSION}.exe" 0
  ...
  WriteRegStr HKCU "Software\Classes\faircoin" "DefaultIcon" "$\"$INSTDIR\electrumfairchains.ico, 0$\""
  WriteRegStr HKCU "Software\Classes\faircoin\shell\open\command" "" "$\"$INSTDIR\electrumfairchains.exe$\" $\"%1$\""
  ...
  WriteRegStr HKCU "${PRODUCT_UNINST_KEY}" "DisplayIcon" "$INSTDIR\efc.ico"
